    ...
    <button onClick={() => navigate(-1)} style={{ color: 'var(--text-dim)', textDecoration: 'none', fontSize: 14, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>← Назад</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20 }}>
        ...
        <div>
          ...
          {donorInfo && (
          <div className="card" style={{ padding: '10px 14px', marginTop: 12, display: 'flex', gap: 16, alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {product.donor_code && <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 2 }}>Код: {product.donor_code}</div>}
              {product.donor_vin && <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>VIN: {product.donor_vin}</div>}
              {(product.donor_body_type || product.donor_engine_code || product.donor_engine_volume) && (
                <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>
                  {product.donor_body_type}{product.donor_body_type && (product.donor_engine_code || product.donor_engine_volume) ? ' · ' : ''}
                  {product.donor_engine_code}{product.donor_engine_code && product.donor_engine_volume ? ' ' : ''}{product.donor_engine_volume ? product.donor_engine_volume + 'л' : ''}
                </div>
              )}
              {(product.donor_fuel_type || product.donor_transmission) && (
                <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>
                  {product.donor_fuel_type}{product.donor_fuel_type && product.donor_transmission ? ' · ' : ''}{product.donor_transmission}
                </div>
              )}
              {product.donor_mileage != null && <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>Пробег: {Number(product.donor_mileage).toLocaleString('ru-RU')} км</div>}
            </div>
            <div style={{ flexShrink: 0, minWidth: 110 }}>
              <div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 4 }}>ВЫГРУЗКА</div>
