...
async function pollActiveDeliveries() {
  ...
  for (const deal of deals) {
    try {
      ...
      if (delivered) {
        const [[dealRow]] = await db.query('SELECT paid, total FROM deals WHERE id=?', [deal.id]);
        const isPaidInFull = dealRow && Number(dealRow.paid || 0) >= Number(dealRow.total || 0);
        if (isPaidInFull) {
          await db.query(
          ...
          await db.query(
            'UPDATE deals SET delivery_status=?, delivery_events=?, delivered_at=NOW() WHERE id=?',
            [statusText, eventsJson, deal.id]
          );
        }
        await db.query(
        ...
        console.log('Deal ' + deal.id + (isPaidInFull ? ' auto-completed: delivered and paid.' : ' delivered but NOT completed: payment pending.'));
      } else {
