...
function buildPrompt(oemNumber, productName, brand, donorInfo) {
  const donorLine = donorInfo && donorInfo.make
    ? `KNOWN FACT (do not doubt this): this exact physical part was removed from a ${donorInfo.make} ${donorInfo.model || ''} ${donorInfo.year || ''}. This vehicle MUST be the first entry in your applicability list. If you have knowledge that this OEM number is also used on other vehicles (e.g. shared platform siblings, rebadged versions), add those as additional rows, but the donor vehicle above always comes first and is never omitted.`
    : 'No donor vehicle info provided.';
  return [
    ...
    '2) a list of vehicle make/model/generation combinations that use this exact part, each as a separate row, starting with the known donor vehicle above if provided.',
    '',
    ...
    '- Include only entries you are genuinely confident about (up to 20 max), plus the mandatory donor vehicle above if one was given.',
    ...
    'If you cannot determine any applicability or cross references beyond the known donor vehicle, return the donor vehicle alone in applicability and an empty array for cross_references.',
  ].join('\n');
}

async function lookupOemCrossAndApplicability(oemNumber, productName, brand, donorInfo) {
  if (!OEM_PROXY_URL || !OEM_PROXY_TOKEN) throw new Error('OEM_PROXY_URL/TOKEN not set');
  const prompt = buildPrompt(oemNumber, productName, brand, donorInfo);
  ...
  const crossRefs = Array.isArray(parsed.cross_references) ? parsed.cross_references
    ...
    : [];
  let applicability = Array.isArray(parsed.applicability) ? parsed.applicability
  ...
  // Server-side guarantee: the donor vehicle is always in the list, regardless of what the AI said.
  if (donorInfo && donorInfo.make) {
    const norm = s => String(s || '').toLowerCase().replace(/[^a-zа-я0-9]/gi, '');
    const alreadyHasDonor = applicability.some(a =>
      norm(a.make) === norm(donorInfo.make) && norm(a.model) === norm(donorInfo.model)
    );
    if (!alreadyHasDonor) {
      applicability.unshift({
        make: donorInfo.make,
        model: donorInfo.model || '',
        generation: null,
        years: donorInfo.year ? String(donorInfo.year) : null,
        body_codes: null,
        engine: null,
      });
    }
