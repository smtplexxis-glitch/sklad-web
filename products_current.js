...
router.post('/:id/ai-lookup', requireRole('admin', 'storekeeper', 'manager'), async (req, res) => {
  try {
    const [[prod]] = await db.query('SELECT id, name, oem_number, oem_manufacturer FROM products WHERE id=?', [req.params.id]);
    if (!prod) return res.status(404).json({ error: 'Product not found' });
    if (!prod.oem_number) return res.status(400).json({ error: 'Product has no OEM number' });
    const result = await lookupOemCrossAndApplicability(prod.oem_number, prod.name, prod.oem_manufacturer);
    let added = 0;
